...
export class LightAccessory {
  ...
  static async create(
    ...
    if (supportsBrightness) {
      try {
        ...
        if (level != null) {
          brightness = level;
          platform.log.debug(`Initialized brightness ${accessory.UUID} ${brightness}`);
        }
      } catch {
  ...
  private constructor(
    ...
    this.service
      ...
      .onSet(async (value) => {
        ...
        this.platform.log.info(`Set status ${value} for ${this.accessory.UUID} ${this.address} ${this.eoj}`);
      })
      .onGet(async () => {
        this.platform.log.info(`Getting status from ${this.accessory.UUID} ${this.address} ${this.eoj}`);
        try {
          const status = (await this.el.getPropertyValue(this.address, this.eoj, 0x80)).message.data?.status;
          if (status != null) {
            this.lastStatus = status;
            this.platform.log.info(`Got status: ${this.lastStatus}`);
          }
        } catch (err) {
          this.platform.log.debug(`Failed to get status from ${this.accessory.UUID}: ${err}`);
        }
      ...
      this.service
        ...
        .onSet(async (value) => {
          if (value !== this.brightness) {
            this.platform.log.debug(`Setting brightness ${this.accessory.UUID} ${value} ${this.address} ${this.eoj}`);
            this.updateBrightness(value as number);
            await this.el.setPropertyValue(this.address, this.eoj, 0xb0, { level: value as number });
          } else {
            this.platform.log.debug(
              `Setting brightness no-op ${this.accessory.UUID} ${value} ${this.address} ${this.eoj}`,
            );
          }
        })
        .onGet(() => {
          this.platform.log.debug(`Getting brightness ${this.accessory.UUID} ${this.address} ${this.eoj}`);
    ...
    el.onNotify((res) => {
      ...
      this.platform.log.info("Received a notification from the device");
      ...
      for (const p of prop ?? []) {
        ...
        if (p.epc === 0x80 && p.edt.status != null) {
          ...
          this.platform.log.info(`Received and updated status: ${p.edt.status}`);
  ...
  private async refreshBrightness(): Promise<void> {
    try {
      ...
      this.platform.log.debug(`Got brightness ${this.accessory.UUID} ${this.address} ${this.eoj}`);
      if (level != null) {
        this.updateBrightness(level);
      }
    } catch (err) {
      this.platform.log.debug(`Failed to get brightness from ${this.accessory.UUID}: ${err}`);
    }
  }
}
