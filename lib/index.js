...
module.exports = (api) => {
  // Read settings.
  try {
    storagePath = path.join(api.user.storagePath(), "persist", `${PLATFORM_NAME}.json`);
    storage = JSON.parse(fs.readFileSync(storagePath));
  } catch {}

  // Register the platform.
  api.registerPlatform(PLUGIN_NAME, PLATFORM_NAME, ELPlatform, true);
};
...
class ELPlatform {
  ...
  configureAccessory(accessory) {
    ...
    if (accessory.UUID === kRefreshUUID) {
      ...
      else this.api.unregisterPlatformAccessories(PLUGIN_NAME, PLATFORM_NAME, [accessory]);
      return;
    }
  ...
  async _buildRefreshAccessory() {
    if (!this.refreshSwitch) {
      ...
      this.api.registerPlatformAccessories(PLUGIN_NAME, PLATFORM_NAME, [this.refreshSwitch]);
    }
    this.refreshService =
