...
class ELPlatform {
  ...
  async _init() {
    ...
    if (this.accessories.size === 0) {
      ...
      for (const [uuid, accessory] of this.accessories) {
        ...
        console.log("Adding", info.address, info.eoj, accessory.UUID);
        if (info) {
  ...
  async _startDiscovery() {
    ...
    return new Promise((resolve, reject) => {
      el.startDiscovery(async (err, res) => {
        ...
        console.log("3Address", address);

        for (const eoj of device.eoj) {
          // Invalid device.
          console.log("4EOJ", eoj);
  ...
  async _buildRefreshAccessory() {
    ...
    this.refreshService =
      this.refreshSwitch.getService(hap.Service.Switch) || this.refreshSwitch.addService(hap.Service.Switch);
    this.refreshService
  ...
  async _addAccesory(address, eoj, uuid) {
    ...
    let accessory = registered ? this.accessories.get(uuid) : new Accessory(el.getClassName(eoj[0], eoj[1]), uuid);

    // The _addAccesory may be called twice due to refreshing.
    if (!accessory.alreadyBuilt) {
      if (!(await buildAccessory(hap, accessory, el, address, eoj))) return; // unsupported accessory
