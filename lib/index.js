...
const REJOIN_INTERVAL_MS = 20 * 1000;
  ...
  _startRenewalTimer() {
    rejoinIntervalId = setInterval(() => {
      try {
        ...
        let netifList = el.el.mELNet._getNetIfList();
        netifList.forEach((netif) => {
          console.log(`Renewing ${netif}`);
          el.el.mELNet.udp.addMembership(el.el.mELNet.ENL_MULTICAST_ADDRESS, netif);
        });

        this.log.info("Renewed multicast group membership");
      } catch (error) {
        this.log.error(`Failed to renew multicast group membership: ${error}`);
      }
    }, REJOIN_INTERVAL_MS);
    this.log.info("Started multicast group rewneal timer");
  }
